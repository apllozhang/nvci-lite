...
分析完成后网页直接渲染参数对照矩阵（分组行 × 产品列）。「待核对」与「已人工确认」单元格可点开核对弹窗：

- 查看该字段的**全部候选与原文引用**（规则 / AI / 视觉模型来源标注），单选定值或直接修正填写
- 可选填**型号**（多型号共用一份彩页时注明归属）与备注
- 确认按「资料 × 字段」保存，绑定**确认时的彩页 SHA-256**：下次分析自动采用（矩阵与 Excel/Word 导出同步生效）；彩页更新后自动失效并提示复核，不会拿旧结论冒充新事实
- 「清除确认」可还原机器原始结论（保留覆盖前快照）

确认记录存于 `data/confirmations.json`，接口：`GET/POST/DELETE /api/confirmations`。
