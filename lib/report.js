...
async function buildExcel({ matrix, documents, outPath }) {
  ...
  sheet.columns = [
    { key: 'field', width: 22 },
    { key: 'group', width: 12 },
    ...matrix.documents.map((doc) => ({ key: doc.documentId, width: 30 })),
  ];
  ...
  for (const group of matrix.groups) {
    ...
    for (const field of group.fields) {
      ...
      for (const doc of matrix.documents) {
        ...
        row[doc.documentId] = cell.status === 'ok' || cell.status === 'pending_review' ? cell.value
          : (cell.status === 'extract_failed' ? '（抽取失败）' : '（未披露）');
      }
      const added = sheet.addRow(row);
      for (const doc of matrix.documents) {
        const value = field.values[doc.documentId];
        const status = value.status;
        const cell = added.getCell(doc.documentId);
        if (status === 'not_disclosed') { cell.font = { color: { argb: 'FF808080' }, italic: true }; }
          ...
          cell.note = `待复核：${origin}抽取值，未经原文引用校验，请人工核对后再采用`;
        }
      }
    }
  }
  sheet.addRow({});
  const noteRow = sheet.addRow({ field: '说明：「未披露」指该型号彩页未写明该参数，不代表不支持；「抽取失败」指文本已抽取但规则未识别，请查看「原文片段」人工核对；蓝色斜体「待复核」为视觉/AI 抽取且未经原文校验的推测值，须人工核对后方可作为决策依据。' });
    ...
    for (const field of group.fields) {
      for (const doc of matrix.documents) {
        ...
        if (cell.status === 'ok' && cell.quote) {
          quoteSheet.addRow({ label: doc.label, field: field.label, value: cell.value, quote: cell.quote, page: cell.page || '', source: cell.source || '' });
        }
      }
    }
  }
...
async function buildWordDocx({ analysis, matrix, documents, outPath }) {
  ...
  children.push(paragraph('完整参数对照、资料清单与原文片段见同批生成的 Excel 文件。标注「未披露」表示该型号公开彩页未写明该参数，不代表不支持，采购时应向厂商验证。', { color: '808080' }));
...
function buildMaterialPack({ matrix, documents, extractions, outPath }) {
  ...
  lines.push('1. 结论只基于给定参数；参数为「未披露」时明确说明彩页未披露，禁止推断为不支持。');
  lines.push('2. 标注「待复核」的值是机器推测值（未经原文校验），引用时须注明待人工复核，重要结论不得单独依赖。');
  lines.push('3. 指出硬门槛差异（端口/PoE/交换容量等一票否决项）、关键偏离、适用场景。');
  lines.push('4. 采购验证问题要具体可执行。');
  lines.push('5. 输出章节：执行摘要 / 硬门槛差异 / 逐参数对比 / 关键偏离 / 适用场景建议 / 采购验证问题清单。');
  lines.push('```');
  lines.push('');
