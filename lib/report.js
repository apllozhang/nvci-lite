...
const { cellStatusText, FEATURE_GROUP, PERF_FIELD_KEYS } = require('./params');

function cellDisplay(cell) {
  if (!cell) return '（未找到）';
  if (cell.status === 'ok' || cell.status === 'pending_review') return cell.value || '';
  return `（${cellStatusText(cell.status)}）`;
}
...
function styleStatusCell(cell, value) {
  ...
  if (status === 'not_disclosed') { cell.font = { color: { argb: 'FF808080' }, italic: true }; }
    ...
    cell.font = { color: { argb: 'FF00857D' }, bold: true };
  }
}

// 竞品对标两张矩阵：性能横比 + 特性勾选（借鉴 competitor-product-tracker 密度）
function addPerfMatrixSheet(workbook, matrix) {
  const sheet = workbook.addWorksheet('性能参数矩阵', { views: [{ state: 'frozen', xSplit: 1, ySplit: 2 }] });
  sheet.columns = [
    { key: 'field', width: 18 },
    ...matrix.documents.map((doc) => ({ key: doc.columnId || doc.documentId, width: 28 })),
  ];
  sheet.mergeCells(1, 1, 1, 1 + matrix.documents.length);
  const title = sheet.getCell(1, 1);
  title.value = `性能参数横向对比 · 生成于 ${new Date().toLocaleString('zh-CN')} · 「未找到」≠ 不支持`;
  title.font = { bold: true, size: 12, color: { argb: 'FFFFFFFF' } };
  title.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF2F5496' } };
  title.alignment = { horizontal: 'center' };
  const header = sheet.getRow(2);
  header.values = ['参数', ...matrix.documents.map((doc) => doc.label)];
  header.font = { bold: true };
  header.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFD6DCE4' } };

  const byKey = new Map();
  for (const group of matrix.groups) {
    for (const field of group.fields) byKey.set(field.key, field);
  }
  for (const key of PERF_FIELD_KEYS) {
    const field = byKey.get(key);
    if (!field) continue;
    const row = { field: field.label };
    for (const doc of matrix.documents) {
      const columnId = doc.columnId || doc.documentId;
      row[columnId] = cellDisplay(field.values[columnId]);
    }
    const added = sheet.addRow(row);
    for (const doc of matrix.documents) {
      const columnId = doc.columnId || doc.documentId;
      styleStatusCell(added.getCell(columnId), field.values[columnId]);
    }
  }
  const note = sheet.addRow({ field: '说明：定位/OS/最高速率/静态·满载功耗/插槽等用于竞品横扫；证据与待核对状态见「参数对照」与「原文片段」。' });
  note.font = { color: { argb: 'FF808080' }, size: 9 };
}

function addFeatureMatrixSheet(workbook, matrix) {
  const sheet = workbook.addWorksheet('功能特性矩阵', { views: [{ state: 'frozen', xSplit: 1, ySplit: 2 }] });
  sheet.columns = [
    { key: 'feature', width: 16 },
    ...matrix.documents.map((doc) => ({ key: doc.columnId || doc.documentId, width: 14 })),
  ];
  sheet.mergeCells(1, 1, 1, 1 + matrix.documents.length);
  const title = sheet.getCell(1, 1);
  title.value = `功能特性矩阵（✓ 支持 · △ 部分 · ✗ 明确不支持 · 未找到=资料未写明）`;
  title.font = { bold: true, size: 12, color: { argb: 'FFFFFFFF' } };
  title.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF548235' } };
  title.alignment = { horizontal: 'center' };
  const header = sheet.getRow(2);
  header.values = ['特性', ...matrix.documents.map((doc) => doc.label)];
  header.font = { bold: true };
  header.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFE2EFDA' } };

  const featureGroup = matrix.groups.find((g) => g.group === FEATURE_GROUP);
  const fields = featureGroup?.fields || [];
  if (!fields.length) {
    const empty = sheet.addRow({ feature: '（本次未抽到特性勾选，见参数对照）' });
    empty.font = { color: { argb: 'FF808080' }, italic: true };
  }
  for (const field of fields) {
    const row = { feature: field.label };
    for (const doc of matrix.documents) {
      const columnId = doc.columnId || doc.documentId;
      const cell = field.values[columnId];
      if (!cell || cell.status === 'not_disclosed') row[columnId] = '未找到';
      else if (cell.status === 'pending_review') row[columnId] = `${cell.value || '?'}?`;
      else row[columnId] = cell.value || '未找到';
    }
    const added = sheet.addRow(row);
    for (const doc of matrix.documents) {
      const columnId = doc.columnId || doc.documentId;
      const cell = added.getCell(columnId);
      const raw = field.values[columnId];
      if (raw?.value === '✓') cell.font = { color: { argb: 'FF00857D' }, bold: true };
      else if (raw?.value === '△') cell.font = { color: { argb: 'FFBF8F00' }, bold: true };
      else if (raw?.value === '✗') cell.font = { color: { argb: 'FFC00000' }, bold: true };
      else cell.font = { color: { argb: 'FF808080' }, italic: true };
      cell.alignment = { horizontal: 'center' };
    }
  }
  const note = sheet.addRow({ feature: '说明：仅彩页明确写出「不支持」才标 ✗；未写明一律「未找到」，不得推断为不支持。部分支持标 △。' });
  note.font = { color: { argb: 'FF808080' }, size: 9 };
}

// Excel 参数对照表：参数对照 / 性能矩阵 / 特性矩阵 / 资料清单 / 原文片段（+门槛）。
async function buildExcel({ matrix, documents, outPath }) {
  ...
  addPerfMatrixSheet(workbook, matrix);
  addFeatureMatrixSheet(workbook, matrix);
