...
// 品类推断（第 3 步品类化分组）：profiles 未标 category 时按产品线名关键词兜底，
// 自定义来源与历史数据走此路径；标准品类的中文名/说明由前端 i18n 提供
function inferCategory(name) {
  const text = String(name || '');
  if (/数据中心|datacenter|DC/i.test(text)) return 'dc_switch';
  if (/工业|industrial/i.test(text)) return 'industrial';
  if (/无线|wlan|wi-?fi|ap\b/i.test(text)) return 'wireless_ap';
  if (/路由|rout/i.test(text)) return 'router';
  if (/安全|防火墙|security|firewall/i.test(text)) return 'security';
  if (/管理|manage/i.test(text)) return 'mgmt_platform';
  if (/交换|switch/i.test(text)) return 'campus_switch';
  return 'other';
}
  ...
  for (const { dir, custom } of locations) {
    ...
    for (const name of files) {
      ...
      vendor.productLines.push({
        ...
        category: profile.category || inferCategory(first.productLineName),
        documentCount: documents.length,
...
module.exports = { loadCatalog, findDocuments, defaultProfilesDir, defaultCustomProfilesDir, inferCategory, VENDOR_NAME_FALLBACK };
