...
// 密钥脱敏（评审 R5）：settings.json 的 ai.apiKey 是界面保存的明文密钥，
// 备份文件会躺在 backups/ 目录长期留存，必须脱敏——恢复后空值自动回落环境变量
// （settings 优先级：文件 > env，空字段即用 env）。密钥只经 UI 设置且无 env 兜底的
// 部署，恢复后需重新填写一次。
const SECRET_REDACT_RULES = [
  { file: 'settings.json', pick: (parsed) => parsed?.ai, field: 'apiKey' },
];

function redactSecrets(relative, text, redacted) {
  const rule = SECRET_REDACT_RULES.find((item) => item.file === relative);
  if (!rule) return text;
  let parsed;
  try { parsed = JSON.parse(text); } catch { return text; } // 坏 JSON 交给上层校验报错
  const section = rule.pick(parsed);
  if (section && typeof section === 'object' && section[rule.field]) {
    section[rule.field] = '';
    redacted.push(`${relative}:ai.${rule.field}`);
    return `${JSON.stringify(parsed, null, 2)}\n`;
  }
  return text;
}

// 生成 bundle：{ schemaVersion, createdAt, fileCount, redacted, files: { 相对路径: 原始文本 } }
function createBackup(dataDir, now = new Date()) {
  ...
  for (const relative of relatives) {
    let text = fs.readFileSync(path.join(dataDir, relative), 'utf8');
    JSON.parse(text); // 备份前先验证是合法 JSON：坏文件当场暴露，不带进备份
    text = redactSecrets(relative, text, redacted);
    files[relative] = text;
  }
  ...
  return { fileName, outPath, fileCount: relatives.length, redacted, removed };
}
