...
function evaluateThresholds(matrix, thresholds) {
  ...
  return (Array.isArray(thresholds) ? thresholds : []).map((threshold) => {
    ...
    for (const doc of matrix.documents || []) {
      if (!op) { row.results[doc.documentId] = { verdict: 'unknown', reason: '门槛条件不合法' }; continue; }
      if (!parsed) { row.results[doc.documentId] = { verdict: 'unknown', reason: '门槛值无法解析' }; continue; }
      if (!field) { row.results[doc.documentId] = { verdict: 'unknown', reason: '字段不在矩阵中' }; continue; }
      const cell = field.values[doc.documentId];
      if (!cell || cell.status === 'pending_review') {
        row.results[doc.documentId] = { verdict: 'unknown', reason: cell ? `取值待核对（${cell.reviewNote || '机器推测'}）` : '无取值' };
        continue;
      }
      if (cell.status !== 'ok' || !cell.value) {
        row.results[doc.documentId] = { verdict: 'unknown', reason: '未找到适用值' };
        continue;
      }
      const cellNumber = comparableCellNumber(cell.value);
      if (cellNumber.reason) { row.results[doc.documentId] = { verdict: 'unknown', reason: cellNumber.reason }; continue; }
      const comparison = compareValues(parsed, cellNumber.token);
      if (comparison.reason) { row.results[doc.documentId] = { verdict: 'unknown', reason: comparison.reason }; continue; }
      const pass = op.test(comparison.b, comparison.a);
      row.results[doc.documentId] = {
