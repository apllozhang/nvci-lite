...
// 冲突保留：胜出者与落选者取值不一致时（规则 128 vs AI 256），胜出值降为待核对，
// 并在 reviewNote 中保留两个候选——正则命中不等于型号与语义正确，冲突必须人工裁决。
function mergeParams(primaryParams, secondaryParams) {
  ...
  const insert = (param) => {
    ...
    const winner = evidenceRank(param) > evidenceRank(existing) ? param : existing;
    const loser = winner === param ? existing : param;
    if (normalizeValueText(winner.value) !== normalizeValueText(loser.value)) {
      byKey.set(param.key, {
        ...winner,
        status: 'pending_review',
        reviewNote: `取值冲突：${winner.source || '未知'}=${String(winner.value).slice(0, 60)}；${loser.source || '未知'}=${String(loser.value).slice(0, 60)}`,
      });
    } else {
      byKey.set(param.key, winner);
    }
  };
