...
const FIELD_TEMPLATE = [
  ...
  { key: 'os_version', label: '操作系统', group: '软件', patterns: [/(Comware\s*V?\d+(?:\.\d+)?(?:\s*R\d+)?)/i, /(Cisco\s*IOS(?:\s*(?:XE|XR))?\s*[^\n，。]{0,16}\d{2})/i, /\bAOS\s*[^\n，。]{0,12}/i, /(?:操作系统|OS版本)[^\n]{0,8}([A-Za-z][^\n，。]{2,24})/i] },
  { key: 'max_speed', label: '最高速率', group: '转发性能', patterns: [/(?:最高速率|最高端口速率|up\s*to)[^\d]{0,12}(\d+\s*[TGM](?:bit|b)\/?s?)/i, /(\d+\s*[TGM](?:bit|b)\/s)\s*(?:端口|及以下|以下)/i] },
  ...
  // 交换容量：中英文标签 + Gb/s/Gbps/Gbit/s；Cisco 带宽表无标签行（C9200-24T 128 Gbps 288 Gbps 95.23 Mpps）
  { key: 'switching_capacity', label: '交换容量', group: '转发性能', patterns: [
    /(?:交换容量|背板带宽|switching\s*capacity|max\s*switching(?:\s*ASIC)?)[^\dA-Za-z]{0,24}([0-9.]+\s*[TGM](?:bit|b)\/?s?)/i,
    /([0-9.]+\s*[TGM](?:bit|b)\/?s?)[^\n]{0,24}(?:switching\s*capacity|交换容量|背板)/i,
    /\bC9200[^\n]{0,28}?\s([0-9.]+\s*Gbps)\s+[0-9.]+\s*Gbps\s+[0-9.]+\s*Mpps/i,
  ] },
  // 包转发：包转发率/forwarding/switch frame rate；Cisco 表：第二列 Gbps 后的 Mpps
  { key: 'forwarding_rate', label: '包转发率', group: '转发性能', patterns: [
    /(?:包转发率|转发速率|forwarding\s*(?:rate|performance|capacity)|switch\s*frame\s*rate)[^\d]{0,20}([0-9.]+\s*[TGM]pps)/i,
    /([0-9.]+\s*[TGM]pps)[^\n]{0,20}(?:forwarding|包转发|frame\s*rate)/i,
    /\bC9200[^\n]{0,28}?\s[0-9.]+\s*Gbps\s+[0-9.]+\s*Gbps\s+([0-9.]+\s*Mpps)/i,
  ] },
