...
function buildGroupMatrixBlock(matrix, group) {
...
function splitGroupsIntoSegments(matrix, maxSegmentChars = ANALYSIS_SEGMENT_CHARS) {
  ...
  for (const group of matrix.groups) {
    ...
    current.push({ group: group.group, block });
    currentLen += len;
  }
  if (current.length) segments.push(current);
  return segments;
...
async function analyzeWithAi(matrix, documents, { fetchImpl, maxChars = ANALYSIS_SEGMENT_CHARS } = {}) {
  ...
  const segments = needsSegment
    ...
    : [matrix.groups.map((group, index) => ({ group: group.group, block: groupBlocks[index] }))];
  ...
  for (let index = 0; index < segments.length; index += 1) {
    ...
    try {
      ...
      results.push(normalizeAnalysis(parseJsonContent(content)));
    } catch (error) {
      lastError = error; // 单段失败不放弃整份：继续后续段，全部失败才对外抛错
    }
  }
