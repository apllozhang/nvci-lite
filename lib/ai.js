...
const {
  ...
  extractNumTokens, numTokensEquivalent, normalizeKey,
...
const EXTRACT_PROMPT_REV = 'r5-compete';
const CANONICAL_KEYS = 'positioning(产品定位)、os_version(操作系统)、max_speed(最高速率)、form_factor(外形/机架高度)、port_config(端口配置)、downlink_ports(下行端口数)、uplink_ports(上行端口数)、switching_capacity(交换容量)、forwarding_rate(包转发率)、poe_capability(PoE 能力)、poe_budget(PoE 总功率)、power_static(静态功耗)、power_max(满载功耗)、power_consumption(额定功耗)、expansion_slots(扩展插槽)、mac_table(MAC 地址表)、vlan_count(VLAN 数量)、stacking(堆叠能力)、power_redundancy(电源冗余)、mgmt(管理方式)、operating_temp(工作温度)、latency(时延)、feat_stacking_irf(堆叠/IRF)、feat_mlag(M-LAG)、feat_vxlan_evpn(VXLAN+EVPN)、feat_openflow_sdn(OpenFlow/SDN)、feat_macsec(MACsec)、feat_ptp_1588(1588v2 PTP)、feat_telemetry(Telemetry)、feat_integrated_ac(融合AC)、feat_issu(ISSU)';
...
4. group 从这些里选：场景定位/软件/物理规格/端口/转发性能/供电/可靠性/协议特性/管理/环境适应/功能特性/其他。
5. 最多输出 35 个参数，优先覆盖：产品定位、操作系统、端口配置、最高速率、交换容量、包转发率、静态/满载功耗、插槽、PoE、堆叠、功能特性，其余按重要性取舍。输出不要换行美化，紧凑 JSON。
...
7. 功能特性 key（feat_*）：value 只能是 ✓（明确支持）、△（部分/可选支持）、✗（明确写不支持）。未写明支持与否的不要输出该 key。
8. 输出严格 JSON：{"params":[{"key":"","label":"","group":"","value":"","quote":"","page":1,"seriesWide":false}]}`;

// 模型常见的自造同义 key 归一到字典 key（§5.1：防止"包转发率/转发性能"被拆成两个字段）
// key 同义词与 normalizeKey 已迁移 value-normalize.js（矩阵构建层也要用，防旧缓存分裂），此处经 import 使用。
...
// 长彩页分段抽取（验收实战：ALE 21 页全量输入时模型返回为空）：超过单段上限按
// 页边界切段、逐段抽取，结果 mergeParams 跨段合并——同键同值去重、互异值保留
// 候选进待核对。缓存仍按整份彩页单键，分段是内部实现细节。
const SEGMENT_CHARS = 28000;

function splitPagesIntoSegments(pageTexts, maxSegmentChars = SEGMENT_CHARS) {
  const segments = [];
  let current = [];
  let currentLen = 0;
  for (const pageText of pageTexts) {
    const len = pageText.length + 2;
    if (current.length && currentLen + len > maxSegmentChars) {
...
async function extractParamsWithAi(document, extraction, { maxChars = SEGMENT_CHARS, fetchImpl, targetModel = '', concurrency = 3 } = {}) {
  const pageTexts = extraction.pages.map((page) => `【第 ${page.page} 页】\n${page.lines.join('\n')}`);
  const totalLen = pageTexts.reduce((sum, text) => sum + text.length + 2, 0);
  const segments = totalLen <= maxChars ? [pageTexts] : splitPagesIntoSegments(pageTexts, maxChars);
  ...
  const targetLine = targetModel
    ...
    : `型号：${modelLine}`;
  // 分段并行（业务侧等不起串行 N 段）：限流 3 路，顺序合并保持语义稳定
  ...
  for (let start = 0; start < segments.length; start += concurrency) {
    ...
    const batchResults = await Promise.all(batch.map(async (segmentPages, offset) => {
      const index = start + offset;
      const segmentNote = segments.length > 1
        ? `\n（本段为整份彩页的第 ${index + 1}/${segments.length} 段。只抽取本段文本中出现的参数，本段没有的参数直接省略，禁止推断。）`
        : '';
      const userPrompt = `产品：${document.vendorName} ${document.series}（${targetLine}）
彩页文本（按页标注）：
${segmentPages.join('\n\n').slice(0, maxChars)}${segmentNote}`;
      try {
        ...
        return { index, ok: true, params: Array.isArray(parsed.params) ? parsed.params : [] };
      } catch (error) {
        return { index, ok: false, error };
      }
    }));
    segmentOutcomes.push(...batchResults);
  }
  segmentOutcomes.sort((a, b) => a.index - b.index);
  const segmentResults = segmentOutcomes.filter((item) => item.ok).map((item) => item.params);
  const lastError = [...segmentOutcomes].reverse().find((item) => !item.ok)?.error || null;
  if (!segmentResults.length) throw lastError || new Error('AI 抽取无任何分段成功');
  let mergedRaw = segmentResults[0];
  for (const segment of segmentResults.slice(1)) mergedRaw = mergeParams(mergedRaw, segment);
  ...
  return params.map((param) => {
    ...
    const crossSegmentConflict = param.status === 'pending_review'
      && Array.isArray(param.candidates) && param.candidates.length > 1;
    const cell = {
    ...
    if (crossSegmentConflict) {
      // 分段间同键互异：保留候选与冲突说明，交人工裁决
      cell.status = 'pending_review';
      cell.reviewNote = param.reviewNote || '分段抽取取值冲突';
      cell.candidates = param.candidates;
    }
    return cell;
  });
}
...
// 超大矩阵（H3C 300+ 行）单次输入会导致模型空返回，仿照参数抽取按组分段，再合并与综合。
const ANALYSIS_SEGMENT_CHARS = 28000;
...
const ANALYSIS_SYNTHESIS_SYSTEM_PROMPT = `你是网络设备选型与竞品分析专家。基于已分段完成的对比分析摘要，写出跨品类总览。
规则：
1. 只综合给定摘要，禁止编造未出现的参数或结论。
2. 待核对值的重要结论须保留"待人工核对"提示。
3. 门槛判定中的「未知」不得写成不满足。
4. 输出严格 JSON：{"executive_summary":"…","key_deviations":["…"],"scenario_advice":[{"scenario":"场景","recommendation":"建议与理由"}],"procurement_questions":["…"]}`;

function buildGroupMatrixBlock(matrix, group, { compact = false } = {}) {
  const rows = group.fields.map((field) => {
    const cells = matrix.documents.map((doc) => {
      ...
      if (cell.status === 'ok') {
        const quote = (!compact && cell.quote) ? `｜原文:「${cell.quote.slice(0, 40)}」(第${cell.page}页)` : '';
        const value = compact ? String(cell.value || '').slice(0, 40) : cell.value;
        return `${doc.label}=${value}${scopeNote}${quote}`;
      }
      if (cell.status === 'pending_review') {
        const reason = cell.reviewNote ? `（待核对·${cell.reviewNote.slice(0, compact ? 16 : 40)}）` : '（待核对）';
        const value = compact ? String(cell.value || '').slice(0, 40) : cell.value;
        return `${doc.label}=${value}${scopeNote}${reason}`;
      }
...
function buildDocSummary(documents) {
  return documents.map((doc) => `- ${doc.label}：${doc.vendorName} ${doc.series}，彩页 ${doc.pageCount || '?'} 页，SHA-256 ${String(doc.sha256 || '').slice(0, 12)}…`).join('\n');
}

function splitGroupsIntoSegments(matrix, maxSegmentChars = ANALYSIS_SEGMENT_CHARS) {
  const segments = [];
  let current = [];
  let currentLen = 0;
  for (const group of matrix.groups) {
    const block = buildGroupMatrixBlock(matrix, group);
    ...
    current.push({ group: group.group, groupRef: group, block });
    currentLen += len;
  }
  if (current.length) segments.push(current);
  return segments;
...
function mergeAnalyses(parts) {
  ...
  for (const part of parts) {
    if (part.parameter_analysis.length) merged.parameter_analysis.push(...part.parameter_analysis);
    if (part.hard_gates.length) merged.hard_gates.push(...part.hard_gates);
    for (const item of part.key_deviations) {
      const key = String(item).trim();
      if (key && !deviationSeen.has(key)) {
        deviationSeen.add(key);
        merged.key_deviations.push(key);
      }
    }
    for (const item of part.scenario_advice) {
      if (item && (item.scenario || item.recommendation)) merged.scenario_advice.push(item);
    }
    for (const item of part.procurement_questions) {
      const key = String(item).trim();
      if (key && !questionSeen.has(key)) {
        questionSeen.add(key);
        merged.procurement_questions.push(key);
      }
    }
    if (part.executive_summary.trim()) summaries.push(part.executive_summary.trim());
  }
  merged.executive_summary = summaries.join('\n\n');
  return merged;
}

async function analyzeWithAi(matrix, documents, { fetchImpl, maxChars = ANALYSIS_SEGMENT_CHARS } = {}) {
  const docSummary = buildDocSummary(documents);
  const thresholdNote = buildThresholdNote(matrix);
  const incompleteNote = buildIncompleteNote(matrix);
  const groupBlocks = matrix.groups.map((group) => buildGroupMatrixBlock(matrix, group));
  const totalLen = groupBlocks.reduce((sum, block) => sum + block.length + 2, 0);
  const needsSegment = totalLen > maxChars;
  const segments = needsSegment
    ? splitGroupsIntoSegments(matrix, maxChars)
    : [matrix.groups.map((group, index) => ({ group: group.group, groupRef: group, block: groupBlocks[index] }))];
  ...
  for (let start = 0; start < segments.length; start += concurrency) {
    const batch = segments.slice(start, start + concurrency);
    const batchResults = await Promise.all(batch.map(async (segment, offset) => {
      const index = start + offset;
      const segmentNote = segments.length > 1
        ? `\n\n（本段仅覆盖参数分组：${segment.map((item) => item.group).join('、')}。这是第 ${index + 1}/${segments.length} 段，只分析本段分组内的字段，禁止扩展到未给出的分组。）`
        : '';
      const fullBlocks = segment.map((item) => item.block);
      const compactBlocks = segment.map((item) => buildGroupMatrixBlock(matrix, item.groupRef, { compact: true }));
      const attempts = [
        { blocks: fullBlocks, label: 'full' },
        { blocks: compactBlocks, label: 'compact' },
      ];
      for (const attempt of attempts) {
        const userPrompt = `对比对象：\n${docSummary}\n\n参数矩阵（含原文引用与待核对原因）：\n${attempt.blocks.join('\n\n')}${segmentNote}${thresholdNote}${incompleteNote}`;
        try {
          const content = await chatCompletion({
            messages: [
          ...
          return { index, ok: true, parsed: normalizeAnalysis(parseJsonContent(content)) };
        } catch (error) {
          lastError = error;
        }
      }
      return { index, ok: false, parsed: null };
    }));
    batchResults.sort((a, b) => a.index - b.index);
    for (const item of batchResults) {
      if (item.ok && item.parsed) results.push(item.parsed);
    }
  }
  if (!results.length) throw lastError || new Error('AI 叙事分析无任何分段成功');

  const merged = mergeAnalyses(results);
  if (segments.length === 1) return merged;

  // 多段结果只拼接会丢跨分组总览：用轻量综合调用重写执行摘要/场景/采购问题
  try {
    const synthesisInput = {
      executive_summary: merged.executive_summary,
      key_deviations: merged.key_deviations.slice(0, 30),
      parameter_analysis: merged.parameter_analysis.slice(0, 40),
      hard_gates: merged.hard_gates,
    };
    const content = await chatCompletion({
      messages: [
        { role: 'system', content: ANALYSIS_SYNTHESIS_SYSTEM_PROMPT },
        { role: 'user', content: `分段分析结果（JSON）：\n${JSON.stringify(synthesisInput, null, 2)}${thresholdNote}${incompleteNote}` },
      ],
      json: true,
      fetchImpl,
    });
    const synthesis = normalizeAnalysis(parseJsonContent(content));
    if (synthesis.executive_summary.trim()) merged.executive_summary = synthesis.executive_summary.trim();
    if (synthesis.key_deviations.length) {
      const seen = new Set();
      merged.key_deviations = [...synthesis.key_deviations, ...merged.key_deviations].filter((item) => {
        const key = String(item).trim();
    ...
    if (synthesis.scenario_advice.length) {
      merged.scenario_advice = [...synthesis.scenario_advice, ...merged.scenario_advice];
    }
    if (synthesis.procurement_questions.length) {
      const seen = new Set();
      merged.procurement_questions = [...synthesis.procurement_questions, ...merged.procurement_questions].filter((item) => {
        const key = String(item).trim();
        if (!key || seen.has(key)) return false;
        seen.add(key);
        return true;
