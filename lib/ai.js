...
async function extractParamsWithAi(document, extraction, { maxChars = SEGMENT_CHARS, fetchImpl, targetModel = '' } = {}) {
...
${segments[index].join('\n\n').slice(0, maxChars)}${segmentNote}`;
    try {
      ...
      segmentResults.push(Array.isArray(parsed.params) ? parsed.params : []);
    } catch (error) {
      lastError = error; // 单段失败不放弃整份：继续后续段，全部失败才对外抛错
    }
  }
...
async function analyzeWithAi(matrix, documents, { fetchImpl, maxChars = ANALYSIS_SEGMENT_CHARS } = {}) {
  ...
  for (let index = 0; index < segments.length; index += 1) {
    const segmentNote = segments.length > 1
      ? `\n\n（本段仅覆盖参数分组：${segments[index].map((item) => item.group).join('、')}。这是第 ${index + 1}/${segments.length} 段，只分析本段分组内的字段，禁止扩展到未给出的分组。）`
      : '';
    const fullBlocks = segments[index].map((item) => item.block);
    const compactBlocks = segments[index].map((item) => buildGroupMatrixBlock(matrix, item.groupRef, { compact: true }));
    const attempts = [
    ...
    for (const attempt of attempts) {
      ...
      try {
        const content = await chatCompletion({
          ...
          json: true,
          fetchImpl,
          maxRetries: attempt.label === 'full' ? 1 : 1,
        });
        parsed = normalizeAnalysis(parseJsonContent(content));
        break;
      } catch (error) {
        lastError = error; // 单段失败不放弃整份：继续后续段/压缩重试，全部失败才对外抛错
      }
    }
