...
const { pdfPageCount } = require('./pdf-text');
const fs = require('fs');
...
function inspectPdf(buffer) {
  ...
  // 页数判定不在本函数：PDF 1.5+ 压缩对象流让字节 grep 对页树失明（锐捷 download.do
  // 23 条被误拒的根因），真实页数由调用方经 pdf-text.pdfPageCount 解析取得
  return { title };
}
...
async function collectDocument({ document, store, fetchImpl = fetch, force = false }) {
  ...
  try {
    ...
    const parsedPageCount = await pdfPageCount(buffer); // 真实解析取页数（兼容压缩对象流）
    if (parsedPageCount < 1) throw new GateError('parse_failed', 'PDF 解析不到任何页面，无法通过基础可读性检查');
