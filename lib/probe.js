...
// 轻量探测：仅 HEAD 元数据，不下载。部分站点对 HEAD 返回异常码时以完整校验为准。
async function lightProbeDocument({ document, fetchImpl = fetch }) {
  const context = {
  ...
  try {
    ...
    if (!headOk && (row.status === 403 || row.status === 405)) {
      // WAF 拒绝 HEAD 但放行 GET 时，回退 GET Range 探测（同一 URL、同一声明 UA）
      ...
      if (row.status === 200 || row.status === 206) {
        return { ...row, probeStatus: 'valid_unchanged', detail: 'HEAD 被拒（403/405），GET 回退验证通过' };
      }
    ...
    return { ...row, probeStatus: 'valid_unchanged', detail: '' };
  } catch (error) {
  ...
  async execute(run, documents) {
    ...
    try {
      for (let i = 0; i < documents.length; i += 1) {
        ...
        let result;
        if (run.mode === 'light') {
          result = await lightProbeDocument({ document: effectiveDoc, fetchImpl: this.fetchImpl });
        } else {
